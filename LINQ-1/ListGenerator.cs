...
namespace LINQ_1
{
    ...
    select new Customer()
    {
        CustomerID = (string?)e.Element("id") ?? "empty",
        CustomerName = (string?)e.Element("name") ?? "empty",
        Address = (string?)e.Element("address") ?? "empty",
        City = (string?)e.Element("city") ?? "empty",
        Region = (string?)e.Element("region") ?? "empty",
        PostalCode =    (string?)e.Element("postalcode") ?? "empty",
        Country = (string?)e.Element("country") ?? "empty",
        Phone = (string?)e.Element("phone") ?? "empty",
        Fax = (string?)e.Element("fax") ?? "empty",
        Orders = (
                  from o in e.Element("orders")?.Elements("order") ??Enumerable.Empty<XElement>()
                  select new Order
                  {
                      OrderID = (int?)o.Element("id") ?? 0,
                      OrderDate = (DateTime?)o.Element("orderdate") ?? DateTime.MinValue,
